...
SOKOL_API_IMPL void _sapp_raspi_mouse_event(sapp_event_type type, sapp_mousebutton btn, int x, int y) {
    if (_sapp_events_enabled()) {
        _sapp_init_event(type);
        _sapp.event.mouse_button = btn;
...
_SOKOL_PRIVATE void _sapp_run(const sapp_desc* desc) {
    while(true)
	{
		//printf("_sapp_frame_before\n");
		ImGui_ImplOpenGL3_NewFrame();
		ImGui_ImplCircle_NewFrame(desc->width, desc->height);	
        _sapp_frame();
        glFlush();
        glFinish();		
		//_glSwapWindow();
		//printf("_sapp_frame_after\n");
	}
}
void test_main(int w, int h) {
    int argc = 0;
    char **argv = 0;
	sapp_desc desc = sokol_main(argc, argv);
	desc.width = w;
...
_SOKOL_PRIVATE void _sapp_circle_key_event(sapp_event_type type, int vk, bool repeat) {
    if (_sapp_events_enabled() && (vk < SAPP_MAX_KEYCODES)) {
        _sapp_init_event(type);
        _sapp.event.modifiers = _sapp_win32_mods();
        _sapp.event.key_code = _sapp.keycodes[vk];
